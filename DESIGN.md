...
이후 처리한 것과 아직 남은 것을 나눠 적습니다.

**처리 완료**

| 항목 | 처리 |
|---|---|
| `next.config.ts`의 `typescript.ignoreBuildErrors` | 해제. 이제 `next build`가 타입 검사를 건너뛰지 않고 실제로 수행한다 |
| 히어로 통계가 자리표시 값(1,284 등) | `data/posts.json`에서 실제로 센다 — 질문 수, 후기 수, 달린 댓글 합계. 홈을 `force-dynamic`으로 두어 빌드 시점 숫자가 굳지 않게 했다 |
| `recommend`의 Google Books 표지 fetch가 클라이언트에서 실행 | 서버로 옮기고 하루 단위 캐싱(`revalidate: 86400`). 페이지를 서버 컴포넌트(`page.tsx`)와 클라이언트 UI(`RecommendBrowser.tsx`)로 분리하고, 데이터·타입은 `lib/recommend-data.ts`로 뺐다. 방문자마다 외부 API를 때리던 요청이 사라졌다 |

**추가로 처리한 것 (인증 정상화 · 죽은 상호작용 제거)**

| 항목 | 처리 |
|---|---|
| 세션 API가 500 | 두 가지가 겹쳐 있었습니다. ① `AUTH_SECRET` 미설정 — `.env.example`로 문서화하고 로컬 개발용 키를 생성. ② 키를 넣자 `UntrustedHost`로 오류가 바뀜 — Vercel 밖에서는 Auth.js가 Host 헤더를 신뢰하지 않으므로 `auth.ts`에 `trustHost: true` 추가. 세션 API 200, 로그인 후 `/community` 이동과 헤더 사용자 표시까지 확인 |
| 큐레이션의 `#id` 더미 링크 | 상세 라우트가 없어 눌러도 아무 일이 없었습니다. 링크를 걷어내고 일반 텍스트로 변경 |
| 댓글 "도움돼요"가 무반응 | 눌리는 것처럼 보이지만 아무 동작이 없었습니다. 댓글 작성과 같은 수준으로 로컬 반영(5 → 6 확인). **저장은 아직 안 됩니다** |

**아직 남음** — 모두 백엔드 작업이 선행되어야 합니다.

1. 공감·댓글 수가 `posts.json`의 고정값 — 글 목록의 공감 수는 여전히 표시 전용입니다. 공감 API 필요
2. `CommentSection`이 목 데이터 + 로컬 상태 — 새로고침하면 사라집니다. 댓글 API 필요
3. 큐레이션 상세 라우트 없음 — 지금은 제목과 요약만 보여줍니다
4. 배포 시 `AUTH_SECRET`을 실제 값으로 설정해야 합니다 (`.env.example` 참고)
5. 실제 사용자 대상 가독성·선호도 테스트 미실시

> 히어로 통계가 실데이터로 바뀌면서 숫자가 작아졌습니다(질문 3 · 후기 2 · 답변 62).
> 자리표시 숫자보다 초라해 보이지만, 서비스가 비어 있을 때 1,284건이 있는 것처럼
> 보이는 편이 더 문제라고 판단했습니다.
